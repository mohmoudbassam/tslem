                         ...
                         ->addColumn('actions', function ($user) {
                             $show =
                                 ' <a class="dropdown-item" href="' . route(
                                     'users.request.show',
                                     [ 'user' => $user->id ]
                                 ) . '"><i class="fa fa-eye mx-1"></i> عرض</a>';
                             $accept = '';
                             $reject = '';
                             if( in_array($user->type, [ "design_office", "contractor" ]) ) {
                                 $designerType =
                                     '<a class="dropdown-item view-designer-types-btn" href="#" data-user="' . $user->id . '"><i class="fa fa-list mx-1"></i>التخصصات</a>';
                             } else {
                                 $designerType = "";
                             }
                             if( $user->verified == 0 ) {
                                 $accept =
                                     '<a class="dropdown-item" href="javascript:;" onclick="accept(' . $user->id . ')" ><i class="fa fa-check mx-1"></i>إعتماد</a>';
                                 $reject =
                                     '<a class="dropdown-item rejection-user-btn" href="#" data-user="' . $user->id . '"> <i class="fa fa-times mx-1"></i>رفض</a>';
                             }
