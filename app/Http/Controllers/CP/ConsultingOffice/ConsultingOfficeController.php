...
class ConsultingOfficeController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($order)
            ->addColumn('actions', function ($order) {
                ...
                //$reports_add = '<a class="dropdown-item" href="'.route('consulting_office.report_add_form',$order->id).'"><i class="fa fa-plus"></i>انشاء التقارير </a>';
                $reports_add = '';
                $reports_view = '';
    ...
    public function reports_list(Order $order)
    {
        ...
        return DataTables::of($reports)
            ->addColumn('actions', function ($report) use ($order) {
                $delete = '<a class="dropdown-item" onclick="deleteReport('.$report->id.')" href="javascript:;"><i class="fa fa-trash me-2"></i>حذف  </a>';
                $edit = '<a class="dropdown-item" href="'.route('consulting_office.report_edit_form', ['report' => $report->id]).'"><i class="fa fa-edit me-2"></i>تعديل  التقرير  </a>';
    ...
    public function add_report(Order $order, Request $request)
    {
        ...
        return redirect()->route('consulting_office.reports_view_details', $order)->with('success', 'تمت اضافة التقرير بنجاح');
    }
