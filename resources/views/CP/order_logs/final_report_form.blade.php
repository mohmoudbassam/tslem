<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
        ...
        <form action="'{{route('order_logs.final_report', ['order'=>$model->id])}}'" method="post" id="form_modal" enctype="multipart/form-data">
            @csrf
            <div class="modal-body">
                <div class="row">
                    @foreach(\App\Models\OrderLogs::getFinalReportRules($model) as $input => $rules)
...
<script>
    ...
    $('#form_modal').validate({
        rules: @json(\App\Models\OrderLogs::getFinalReportRules($model)),
        errorElement: 'span',
    ...
    $('.submit_btn').click(function (e) {
        ...
        postData(new FormData($('#form_modal').get(0)), '{{route('order_logs.final_report', ['order'=>$model->id])}}', (data) => {
            if (data.success) {
    ...
    $(() => {
        @foreach(\App\Models\OrderLogs::getFinalReportRules() as $column => $rules)
