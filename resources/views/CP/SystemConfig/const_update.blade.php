...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="add_edit_form" method="post" action="{{route('const.update')}}">
                @csrf
                <input type="hidden" value="{{ $const->id }}" name="id" />
                <div class="row">

                    <div class="col-md-6">
                        <div class="mb-3">
                            <label  for="parnet_id">نوع الثابت</label>
                            <select class="form-control" id="parnet_id" name="parnet_id" value="{{ $const->parnet_id  }}">
                                <option value="">اختر...</option>
                                @foreach($parnets as $parnet)
                                    <option @if($const->parnet_id == $parnet->id) selected @endif value="{{ $parnet->id  }}">{{ $parnet->name  }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="name" value="{{$const->name}}" id="name" placeholder="الإسم">
